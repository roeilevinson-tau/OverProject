...
for test_case in "${test_cases[@]}"; do
    ...
    if [ "$current_analysis_output" == "$prev_analysis_output" ]; then
        ...
        # Increment passed tests counter only if Python test also passed
        if [ "$python_test_passed" = true ] && [ "$c_test_passed" = true ]; then
...
# Calculate and display the percentage of passed tests
if [ $total_tests -gt 0 ]; then
    percentage=$(( (passed_tests * 100) / total_tests ))
    echo -e "${BLUE}Percentage of passed tests: ${YELLOW}$percentage%${NC}"
else
    echo -e "${RED}No tests were run.${NC}"
fi

# Print the final percentage of passed tests
echo -e "${BLUE}Final Percentage of passed tests: ${YELLOW}$percentage%${NC}"
